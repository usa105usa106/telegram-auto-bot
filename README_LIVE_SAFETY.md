...
В этой сборке добавлен отдельный тренд-фильтр для сигналов и автоторговли.

Когда фильтр включён, бот работает так:

1. На рабочем таймфрейме `SIGNAL_TIMEFRAME` ищет обычный сетап по EMA/RSI/MACD/ATR/объёму/пробою.
2. На старшем таймфрейме `TREND_TIMEFRAME` определяет основной тренд.
3. LONG проходит только при бычьем старшем тренде.
4. SHORT проходит только при медвежьем старшем тренде.
5. Если старший ТФ во флэте или тренд неясный, сигнал не отправляется и авто-сделка не открывается.
...
- `/trend` — статус фильтра тренда.
- `/settings` → `🧭 Фильтр тренда` — включить/выключить фильтр и выбрать старший ТФ.

Для LIVE обычно разумно использовать:
...
Если сигналов слишком мало, можно попробовать:
